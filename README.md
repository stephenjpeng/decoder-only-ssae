...
Range and PCA caches use distinct filenames, so both truncation modes can use
the same embedding folder without overwriting each other. The older
`pca_rotation: true` setting remains supported and maps to PCA replacement
semantics; new configurations should use the explicit method and semantics.
...
| `dataloader.pca_rotation` | `False` | Deprecated compatibility alias for PCA replacement mode. Prefer `truncate_embds_method` and `pca_semantics`. |
