...
`run_image_benchmark` exposes three prompt-side entry paths. They are different computations and must not be averaged or substituted for one another:

| Method key | Report / paper label | Conditioning | Use it for |
|---|---|---|---|
| `native_prompt` | **Native text generation** | Prompt text goes directly to the SD3.5 pipeline | Deployment-realistic direct text generation |
| `prompt_only` | **Exact full-embedding round-trip** | The prompt is encoded, then the full `[1,333,4096]` and `[1,2048]` output is passed back unchanged | Isolating the encode/decode entry path from native text |
| `prompt_modified_packed` | **Prompt modification (packed top-k)** | The prompt is re-encoded; only the active top-k coordinates survive and other coordinates use the selected fill policy | Controlled-subspace comparison with SSAE and ridge edits |

Native text and the exact round-trip both set the T5 maximum sequence length to 256. This preserves the stored 333-token stream contract. The key `prompt_only` remains for method-name compatibility. Cache identity changes when the render contract changes, so older pixels cannot silently acquire new semantics.

`prompt_modified_packed` is **not** in the default method set (it roughly doubles prompt-side rendering cost). Opt in:

```bash
python -m evaluation.run_image_benchmark ... \
    --methods gt_embed,ssae_compose,mean_arithmetic,ridge_embed,native_prompt,prompt_only,prompt_modified_packed
```

Every run and cache manifest records each method's runtime conditioning, active top-k, fill policy, and packer fingerprint.
...
Non-SSAE baselines (`gt_embed`, `mean_arithmetic`, `ridge_embed`, `native_prompt`, `prompt_only`, `prompt_modified_packed`) land in a shared cache at `results/bench_baseline_cache/<dataset_id>/`. The identity includes the data, seed, ridge setting, SD3.5 fingerprint, ordered active top-k coordinate fingerprint, fill policy, and cache contract version. Simulated runs bypass shared cache reads and writes. A real run folder holds SSAE outputs plus a `manifest.json` that points at the cache. Comparing several checkpoints on the same holdout only re-renders SSAE:
...
**No holdout-embedding leakage in image space.** When packing an SSAE / baseline prediction into the full SD3.5 conditioning tensor, `run_image_benchmark` fills the ~1.36M non-top-k dims (and the 2048-D pooled tail) with the training-set mean — cached at `<train_folder>/full_embd_mean.pt` after the first run. Earlier versions filled these dims with the holdout row's true embedding, which quietly inflated image-space metrics for every method by carrying most of the target signal into the render. The fill policy is part of the baseline cache key (see the packer fingerprint in `evaluation/sd3_pack.py`), so caches populated under the old policy don't collide with new ones. `evaluation/run_magnitude_sensitivity.py` used to default to oracle fill; it now uses train-mean fill so its magnitude curves can be placed on the same efficacy/collateral axes as the benchmark's rows. Pass `--oracle_fill` only to reproduce a pre-fix artifact, and never mix the two policies in one comparison. Inference notebooks that don't pass `template=` still get oracle fill.
...
- `--locality_drop_one_attr`: zeros the attribute's mask bit for embedding methods. For prompt paths, it removes the phrase, then uses native text, exact round-trip, or packed re-encoding as selected. It writes pre-edit images to `<output_dir>/images_pre_edit/<method>/` and adds pixel locality metrics.
- `--locality_swap_one_attr`: replaces the attribute with a different property in the same category. Each prompt path then uses its own native, round-trip, or packed entry route. It writes swap images to `<output_dir>/images_swapped/<method>/` and adds pixel and CLIP swap metrics.
