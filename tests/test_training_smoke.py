...
def _build_dataset_folder(root: Path) -> Path:
    ...
    folder.mkdir()
    (folder / "embds").mkdir()
    # H5Dataset's __init__ indexes self.files_pooled[0] before the simulated
    # branch, so at least one embds_* folder must exist on disk.
    (folder / "embds" / "embds_0").mkdir()
...
def test_training_one_epoch_smoke(tmp_path: Path, model_name: str) -> None:
